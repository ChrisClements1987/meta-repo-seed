...
def upgrade_subscription_command(args):
    ...
    try:
        success = customer_manager.upgrade_subscription(
            customer_id=args.customer_id,
            new_plan=SubscriptionPlan(args.plan)
        )
...
Examples:
  ...
  python commercial_cli.py usage --org-id ORG_ID
    ...
    create_customer_parser.add_argument('--plan', choices=['startup', 'growth', 'scale', 'enterprise'], 
                                        default='startup', help='Subscription plan')
    create_customer_parser.add_argument('--trial-days', type=int, default=14, help='Trial period in days')
    ...
    create_org_parser.add_argument('--description', default='', help='Organization description')
    create_org_parser.add_argument('--github-username', default='', help='GitHub username')
    create_org_parser.add_argument('--github-org', help='GitHub organization')
    
    # Deploy organization command
    deploy_parser = subparsers.add_parser('deploy', help='Deploy organization infrastructure')
    deploy_parser.add_argument('--org-id', required=True, help='Organization ID')
    deploy_parser.add_argument('--dry-run', action='store_true', help='Preview changes without making them')
    
    # Upgrade subscription command
    upgrade_parser = subparsers.add_parser('upgrade', help='Upgrade customer subscription')
    upgrade_parser.add_argument('--customer-id', required=True, help='Customer ID')
    upgrade_parser.add_argument('--plan', choices=['startup', 'growth', 'scale', 'enterprise'], 
                               required=True, help='New subscription plan')
