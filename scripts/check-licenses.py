...
class LicenseChecker:
    ...
    def check_license_compliance(self) -> bool:
        ...
        for dep in dependencies:
            ...
            for prohibited in self.PROHIBITED_LICENSES:
                if prohibited in normalized_license:
                    ...
                    if (
                        "LGPL" not in normalized_license
                        and "Lesser" not in normalized_license
                    ):
                        is_prohibited = True
                        break
            ...
            if is_prohibited:
                violations.append(
                    f"VIOLATION: {name} {version}: {license_name} (PROHIBITED)"
                )
                continue
            ...
            if name in self.VERIFIED_PACKAGES:
                ...
                print(
                    f"VERIFIED: {name} {version}: {license_name} -> {verified_license}"
                )
                approved_count += 1
                continue
            ...
            if normalized_license == "UNKNOWN":
                unknown_packages.append(
                    f"UNKNOWN: {name} {version}: {license_name} (needs verification)"
                )
                continue
        ...
        if violations:
            ...
            print()
            print(
                "These dependencies must be removed or replaced with MIT-compatible alternatives."
            )
            return False
        ...
        if unknown_packages:
            ...
            print()
            print(
                "These should be investigated and added to VERIFIED_PACKAGES if safe."
            )
