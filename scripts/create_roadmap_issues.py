...
def create_github_issue(feature: Dict[str, str], dry_run: bool = False) -> bool:
    ...
    # Determine labels
    labels = ["enhancement", "roadmap", get_priority_label(feature["priority"])]
...
def filter_features_by_section(
    features: List[Dict[str, str]], section_filter: str = None
) -> List[Dict[str, str]]:
...
def main():
    parser = argparse.ArgumentParser(
        description="Create GitHub issues from roadmap items"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if args.section:
        ...
        print(
            f"🔍 Filtered to {len(features)} features matching section '{args.section}'"
        )
    ...
    print("-" * 50)
    print(
        f"✅ Successfully {'would create' if args.dry_run else 'created'} {success_count}/{len(features)} issues"
    )
