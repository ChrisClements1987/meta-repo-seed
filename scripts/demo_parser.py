...
def demo_parser():
    ...
    print(
        f"   Schema validation: {'✅ Enabled' if schema_info['has_validation'] else '❌ Disabled (fallback mode)'}"
    )
    ...
    if fixtures_dir.exists():
        ...
        if valid_file.exists():
            ...
            try:
                ...
                print(
                    f"   Top-level directories: {', '.join(result.get_top_level_directories())}"
                )
                ...
                print(
                    f"   - Has 'meta-repo/governance': {result.has_directory('meta-repo/governance')}"
                )
                ...
                if governance_files:
                    print(
                        f"   - Files in meta-repo/governance: {', '.join(governance_files[:3])}..."
                    )
