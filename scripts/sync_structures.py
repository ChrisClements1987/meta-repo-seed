...
try:
    from meta_repo_seed.structure_sync import DirectoryStructure, StructureSynchronizer
except ImportError:
    print(
        "Error: Unable to import StructureSynchronizer. Make sure the module is installed."
    )
    sys.exit(1)
    ...
    try:
        ...
        for structure_name in structures:
            try:
                ...
                print(
                    f"    Files: {len(structure.files)}, Subdirs: {len(structure.subdirs)}"
                )
...
def compare_command(args):
    ...
    if not args.structure1 or not args.structure2:
        print(
            "Error: Both --structure1 and --structure2 are required for compare command"
        )
        return 1
    ...
    try:
        ...
        print(
            f"Comparison between '{args.structure1}' and '{args.structure2 or args.source}':"
        )
...
Examples:
  ...
  python sync_structures.py compare --structure1 template1 --structure2 template2
    ...
    scan_parser = subparsers.add_parser(
        "scan", help="Scan a directory to create a structure template"
    )
    scan_parser.add_argument("--source", required=True, help="Source directory to scan")
    scan_parser.add_argument(
        "--structure", help="Name for the structure template (default: directory name)"
    )
    scan_parser.add_argument("--config", help="Configuration file path")

    # Sync command
    sync_parser = subparsers.add_parser(
        "sync", help="Synchronize a structure template to a target directory"
    )
    sync_parser.add_argument(
        "--structure", required=True, help="Structure template name"
    )
    sync_parser.add_argument(
        "--target", required=True, help="Target directory for sync"
    )
    ...
    sync_parser.add_argument(
        "--interactive",
        action="store_true",
        help="Interactively collect template variables",
    )

    # List command
    list_parser = subparsers.add_parser(
        "list", help="List available structure templates"
    )
    list_parser.add_argument("--config", help="Configuration file path")

    # Compare command
    compare_parser = subparsers.add_parser(
        "compare", help="Compare two structure templates or template with directory"
    )
    compare_parser.add_argument(
        "--structure1", required=True, help="First structure template"
    )
    compare_parser.add_argument("--structure2", help="Second structure template")
    compare_parser.add_argument(
        "--source", help="Directory to compare with (alternative to --structure2)"
    )
