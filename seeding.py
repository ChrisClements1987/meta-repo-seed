...
def sanitize_project_name(name: str) -> str:
    ...
    if not re.match(r"^[\w\-_\.]+$", name, re.UNICODE):
        raise ValueError(
            "Project name may only contain letters, numbers, hyphens, underscores, and periods"
        )
...
def get_github_username() -> str:
    ...
    try:
        ...
        if username:
            logger.info(
                f"Detected GitHub username from git config (github.user): {username}"
            )
            return username
...
def safe_open_for_write(path: Path, encoding: str = "utf-8"):
    ...
    if hasattr(os, "O_NOFOLLOW") and os.name != "nt":  # Not available on Windows
        try:
            ...
            fd = os.open(
                str(path), os.O_WRONLY | os.O_CREAT | os.O_TRUNC | os.O_NOFOLLOW
            )
...
def safe_copy_file(source: Path, destination: Path) -> None:
    ...
    if hasattr(os, "O_NOFOLLOW") and os.name != "nt":
        ...
        fd = os.open(
            str(destination), os.O_WRONLY | os.O_CREAT | os.O_TRUNC | os.O_NOFOLLOW
        )
...
def create_file_from_template(
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"Template file contains binary content, copying as-is: {template_path}"
            )
        ...
        logger.error(
            f"Failed to create {destination} from template {template_path}: {e}"
        )
        raise
...
class Configuration:
    ...
    def load_config(self, config_path: Path) -> Dict[str, Any]:
        ...
        if config_path.suffix.lower() not in self.supported_formats:
            raise ValueError(
                f"Unsupported config format. Supported: {', '.join(self.supported_formats)}"
            )
...
class RepoSeeder:
    ...
    def __init__(
        ...
        self.base_path = (
            Path.cwd().parent if Path.cwd().name == "meta-repo-seed" else Path.cwd()
        )
    ...
    def create_base_structure(self):
        ...
        if not self.dry_run:
            ...
            ensure_directory_exists(
                self.project_root / "core-services", "(core-services)"
            )
            ensure_directory_exists(
                self.project_root / "saas-products", "(saas-products)"
            )
            ensure_directory_exists(
                self.project_root / "partner-products", "(partner-products)"
            )
            ensure_directory_exists(
                self.project_root / "charity-products", "(charity-products)"
            )
    ...
    def setup_meta_repo(self):
        ...
        if not self.dry_run:
            ...
            ci_template = (
                self.templates_dir / "github" / "workflows" / "ci.yml.template"
            )
            ci_dest = workflows_path / "ci.yml"
            copy_template_file(ci_template, ci_dest, "(CI workflow)")

            readme_template = (
                self.templates_dir / "github" / "workflows" / "readme-docs.yml.template"
            )
    ...
    def setup_cloud_storage(self):
        ...
        if not self.dry_run:
            ...
            ensure_directory_exists(
                arch_path / "principles", "(architecture principles)"
            )
            ensure_directory_exists(
                arch_path / "adr", "(architecture decision records)"
            )
            ...
            ensure_directory_exists(
                workspace_path / "meeting-minutes", "(meeting minutes)"
            )
            ...
            ensure_directory_exists(
                docs_path / "partner-products", "(partner-products docs)"
            )
            ensure_directory_exists(
                docs_path / "charity-products", "(charity-products docs)"
            )
            ...
            "structure": {
                ...
                "meta-repo": {
                    ...
                    "documentation": {
                        ...
                        "shared-resources": ["templates.md", "glossary.md", "faq.md"],
                    },
                },
                "core-services": {
                    "contented-cms": [],
                ...
                "saas-products": {"sports-league": [], "saas-product-2": []},
                "partner-products": {},
                "charity-products": {},
            },
        }
    ...
    def create_automation_scripts(self):
        ...
        if not self.dry_run:
            ...
            self.create_script_file(
                scripts_path / "initialise_repo.py", "initialise_repo"
            )
            self.create_script_file(
                scripts_path / "enforce_structure.py", "enforce_structure"
            )
            self.create_script_file(
                scripts_path / "generate_readmes.py", "generate_readmes"
            )
...
''',
    ...
    def setup_documentation(self):
        ...
        if not self.dry_run:
            ...
            ensure_directory_exists(
                docs_path / "shared-resources", "(shared resources)"
            )
    ...
    def create_governance_policies(self):
        ...
        policy_templates = ["contributing.md.template", "code-of-conduct.md.template"]

        for template_name in policy_templates:
            template_path = (
                self.templates_dir / "governance" / "policies" / template_name
            )
    ...
    def create_strategy_files(self):
        ...
        strategy_templates = ["vision.md.template", "mission.md.template"]

        for template_name in strategy_templates:
            template_path = (
                self.templates_dir / "cloud-storage" / "strategy" / template_name
            )
    ...
    def create_shared_resource_templates(self):
        ...
        templates_path = (
            self.meta_repo_path / "governance" / "shared-resources" / "templates"
        )
    ...
    def create_infrastructure_templates(self):
        ...
        if not self.dry_run:
            ensure_directory_exists(
                infrastructure_path, "Infrastructure as Code root directory"
            )
        ...
        if not self.dry_run:
            ...
            ensure_directory_exists(
                environments_path, "Environment configurations directory"
            )
        ...
        if not self.dry_run:
            for env in ["dev", "staging", "production"]:
                ...
                ensure_directory_exists(
                    env_path, f"{env.capitalize()} environment configuration"
                )

        # Create main Terraform configuration files
        terraform_templates = ["main.tf", "variables.tf", "outputs.tf", "providers.tf"]

        for template_name in terraform_templates:
            template_path = (
                self.templates_dir / "infrastructure" / "terraform" / template_name
            )
        ...
        for provider, dest_path in provider_configs:
            ...
            create_file_from_template(
                ...
                provider_dest_path,
                self.replacements,
                f"(terraform: {provider.upper()} infrastructure)",
            )
        ...
        for template_name in kubernetes_templates:
            template_path = (
                self.templates_dir / "infrastructure" / "kubernetes" / template_name
            )
        ...
        for template_name in docker_templates:
            template_path = (
                self.templates_dir / "infrastructure" / "docker" / template_name
            )
    ...
    def setup_code_formatting(self):
        ...
        precommit_template_path = (
            self.templates_dir / "code-formatting" / ".pre-commit-config.yaml"
        )
        ...
        pyproject_template_path = (
            self.templates_dir / "code-formatting" / "pyproject.toml"
        )
        ...
        requirements_template_path = (
            self.templates_dir / "code-formatting" / "requirements-formatting.txt"
        )
        requirements_dest_path = self.meta_repo_path / "requirements-formatting.txt"
        if not self.dry_run:
            create_file_from_template(
                requirements_template_path,
                requirements_dest_path,
                self.replacements,
        ...
        setup_script_template_path = (
            self.templates_dir / "code-formatting" / "setup-formatting.py"
        )
        setup_script_dest_path = scripts_path / "setup-formatting.py"
        if not self.dry_run:
            create_file_from_template(
                setup_script_template_path,
                setup_script_dest_path,
                self.replacements,
                "(formatting setup script)",
            )
        ...
        if not self.dry_run:
            ...
            logger.info(
                "    💡 Run 'python scripts/setup-formatting.py' to configure your environment"
            )
    ...
    def create_github_repository_settings(self):
        ...
        settings_template_path = (
            self.templates_dir / "github-settings" / "repository-settings.yml"
        )
        ...
        script_template_path = (
            self.templates_dir / "github-settings" / "apply-github-settings.py"
        )
        ...
        workflow_template_path = (
            self.templates_dir / "github-settings" / "repository-settings-workflow.yml"
        )
        ...
        if not self.dry_run:
            ...
            logger.info(
                "    💡 Run 'python scripts/apply-github-settings.py' to configure your repository"
            )
    ...
    def create_audit_management_system(self):
        ...
        if not self.dry_run:
            logger.info(
                "🔍 Creating audit management and AI agent coordination system..."
            )
        ...
        registry_template_path = (
            self.templates_dir / "audit-management" / "audit-registry.yaml"
        )
        ...
        for agent_config in agent_configs:
            template_path = (
                self.templates_dir / "audit-management" / "agents" / agent_config
            )
        ...
        for template_name in audit_templates:
            template_path = (
                self.templates_dir / "audit-management" / "templates" / template_name
            )
            ...
            if not self.dry_run:
                create_file_from_template(
                    ...
                    dest_path,
                    self.replacements,
                    f"(audit template: {template_name})",
                )
        ...
        for script_name in script_files:
            template_path = (
                self.templates_dir / "audit-management" / "scripts" / script_name
            )
        ...
        tracking_files = [
            ...
            (
                ...
                tracking_path / "implementation-status.yaml",
            ),
        ]

        for template_name, dest_path in tracking_files:
            template_path = (
                self.templates_dir / "audit-management" / "tracking" / template_name
            )
            if not self.dry_run:
                create_file_from_template(
                    template_path,
                    dest_path,
                    self.replacements,
                    f"(audit tracking: {template_name})",
                )
        ...
        if not self.dry_run:
            ...
            logger.info(
                "    💡 Run 'python scripts/audit-manager.py' to coordinate audits"
            )
...
Configuration file examples:
  ...
  python seeding.py --list-configs                  # List available config files
    ...
    # Basic options
    parser.add_argument(
        "--project", type=str, help="Project name (default: auto-detect from directory)"
    )
    ...
    parser.add_argument(
        "--config", type=Path, help="Load configuration from YAML or JSON file"
    )
    ...
    parser.add_argument(
        "--template-path", type=Path, help="Custom path to templates directory"
    )
...
def main():
    ...
    try:
            ...
            if config_files:
                ...
                for config_file in config_files:
                    try:
                        ...
                        logger.info(
                            f"  {config_file} - Project: {project_name}, Created: {created_at}"
                        )
                    except Exception as e:
        ...
        raw_project_name = (
            args.project
            or (config.get_project_name() if config_data else None)
            or get_project_name()
        )
        ...
        github_username = (
            args.username
            or (config.get_github_username() if config_data else None)
        ...
        if args.template_path:
            if args.template_path.exists():
                ...
                logger.warning(
                    f"Template path does not exist, using default: {seeder.templates_dir}"
                )
