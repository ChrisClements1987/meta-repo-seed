...
class PortfolioConfig:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "PortfolioConfig":
        ...
        repositories = [
            RepositoryConfig.from_dict(repo) for repo in data.get("repositories", [])
        ]
        environments = [
            EnvironmentConfig.from_dict(env) for env in data.get("environments", [])
        ]
...
class OrganizationSpec:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "OrganizationSpec":
        ...
        return cls(structure=structure, teams=teams, settings=data.get("settings", {}))
...
class OrganizationBlueprint:
    ...
    def get_repositories_by_portfolio(
...
class BlueprintParser:
    ...
    def __init__(self, schema_path: Optional[Path] = None):
        ...
        if schema_path is None:
            schema_path = (
                ...
                / "schemas"
                / "organization-blueprint-v1.json"
            )
    ...
    def parse_file(self, blueprint_path: Union[str, Path]) -> OrganizationBlueprint:
        ...
        try:
            ...
            self.logger.info(
                f"Successfully parsed blueprint: {blueprint.metadata.name}"
            )
            return blueprint
    ...
    def validate_blueprint(self, blueprint: OrganizationBlueprint) -> List[str]:
        ...
        if not blueprint.metadata.business_profile:
            warnings.append(
                "No business profile specified - consider adding businessProfile"
            )
        ...
        prod_envs = [
            env for env in blueprint.get_all_environments() if env.type == "production"
        ]
        for env in prod_envs:
            if not env.protection.get("requiredReviewers"):
                warnings.append(
                    f"Production environment '{env.name}' has no required reviewers"
                )
    ...
    def get_deployment_order(self, blueprint: OrganizationBlueprint) -> List[str]:
        ...
        if blueprint.organization.structure.platform_services:
            deployment_order.append(
                blueprint.organization.structure.platform_services.name
            )
    ...
    if examples_dir.exists():
        for blueprint_file in examples_dir.glob("*.yaml"):
            try:
                ...
                logging.warning(
                    f"Failed to load example blueprint {blueprint_file}: {e}"
                )
