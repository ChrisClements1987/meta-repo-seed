...
def deploy_business_command(args) -> int:
    ...
    if args.profile not in BusinessDeploymentProfiles.list_profiles():
        ...
        for profile_name in BusinessDeploymentProfiles.list_profiles():
            logger.info(
                f"  {profile_name}: {BusinessDeploymentProfiles.get_profile_description(profile_name)}"
            )
        return 1
    ...
    async def run_deployment():
        ...
        success, report = await deploy_business_from_profile(
            profile_name=args.profile, org_name=org_name, dry_run=args.dry_run
        )
        ...
        if success:
            ...
            if duration < 10:
                logger.info(
                    "✅ Business infrastructure deployment completed successfully!"
                )
            else:
                logger.info(
                    f"✅ Business infrastructure deployment completed in {duration:.1f} minutes!"
                )

            logger.info(
                f"   Repositories created: {report.get('repositories_created', 0)}"
            )
            logger.info(f"   Teams created: {report.get('teams_created', 0)}")
            logger.info(
                f"   Environments configured: {report.get('environments_configured', 0)}"
            )
    ...
    async def run_product_launch():
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if not generation_result["success"]:
                logger.error(
                    f"❌ Template generation failed: {generation_result.get('error')}"
                )
                return 1
            ...
            if args.dry_run:
                ...
                logger.info(
                    f"  ✓ Technologies: {', '.join(template_config.technologies)}"
                )
            ...
            if deployment_result.is_success:
                ...
                logger.info(
                    f"   Deployment time: {deployment_result.deployment_time_seconds:.1f}s"
                )
    ...
    async def run_onboarding():
        ...
        try:
            ...
            if results["overall_success"]:
                ...
                logger.info(
                    f"   Workflows Created: {results['deployment_summary']['workflows_created']}"
                )
                logger.info(
                    f"   Policies Applied: {results['deployment_summary']['policies_applied']}"
                )
                logger.info(
                    f"   Governance Files: {results['deployment_summary']['governance_files']}"
                )
                ...
                if args.dry_run:
                    ...
                    logger.info("Next steps:")
                    logger.info(
                        "1. Review the compliance checklist issue that will be created"
                    )
                    logger.info(
                        "2. Customize governance policies in /governance/ as needed"
                    )
                    logger.info("3. Monitor the automated workflows in action")
                    logger.info(
                        "4. Check reports/business-operations/ for deployment details"
                    )
...
infrastructure to focus on their core business instead of technology setup.
    ...
    subparsers = parser.add_subparsers(
        dest="command", help="Business deployment commands", metavar="COMMAND"
    )
    ...
    launch_parser.add_argument(
        "--stack",
        choices=ProductStackTemplates.list_stacks(),
        required=True,
        help="Product stack template",
    )
    launch_parser.add_argument("--name", required=True, help="Product name")
    launch_parser.add_argument(
        "--dry-run", action="store_true", help="Preview launch without making changes"
    )
    launch_parser.add_argument(
        "--verbose", "-v", action="store_true", help="Enable detailed logging"
    )
    ...
    onboard_parser.add_argument(
        "--org-name", help="Organization name (defaults to current directory)"
    )
    onboard_parser.add_argument(
        "--repo-name", help="Repository name (defaults to current directory)"
    )
    onboard_parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Preview onboarding without making changes",
    )
    onboard_parser.add_argument(
        "--verbose", "-v", action="store_true", help="Enable detailed logging"
    )
