...
class CustomerSettings:
    ...
    def __post_init__(self):
        ...
        if self.plan == SubscriptionPlan.STARTUP:
            ...
            self.features_enabled = ["basic_templates", "github_integration", "api_access", "webhooks"]
        elif self.plan == SubscriptionPlan.SCALE:
            self.max_products = 20
            self.max_team_members = 100
            self.features_enabled = ["all_templates", "github_integration", "api_access", "webhooks", "analytics"]
...
class Customer:
    ...
    settings: CustomerSettings = field(default_factory=lambda: CustomerSettings(SubscriptionPlan.STARTUP, 1, 5))
...
class Subscription:
    ...
    def __post_init__(self):
        ...
        if self.current_period_end == datetime.now():
            from datetime import timedelta
            self.current_period_end = self.current_period_start + timedelta(days=30)  # Monthly billing
