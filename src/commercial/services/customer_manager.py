...
class CustomerManager:
    ...
    def __init__(self, database_path: Optional[str] = None):
        ...
        self.database_path = database_path or "data/customers.db"
    ...
    def create_customer(
        ...
        Returns:
            Customer object
        """
        self.logger.info(f"Creating new customer: {email} ({company_name})")
        ...
        # Set trial period
        customer.trial_ends_at = datetime.now() + timedelta(days=trial_days)
        
        # Save to database
        self._save_customer(customer)
        
        self.logger.info(f"Customer created successfully: {customer.customer_id}")
        return customer
    
    def get_customer(self, customer_id: str) -> Optional[Customer]:
        """Get customer by ID."""
        return self._load_customer(customer_id)
    ...
    def update_organization(self, organization: Organization) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Failed to update organization {organization.org_id}: {e}")
            return False
    ...
    def upgrade_subscription(
        ...
        self.logger.info(f"Upgrading customer {customer_id} from {old_plan.value} to {new_plan.value}")
        
        # Update customer plan and settings
        customer.plan = new_plan
        customer.settings = customer.settings.__class__(new_plan, 1, 5)  # Reset with new plan
