...
from seeding import (
    setup_logging, sanitize_project_name, ensure_directory_exists,
    safe_open_for_write, safe_copy_file, copy_template_file,
    process_template_content, create_file_from_template, Configuration
)

# Import commercial models
from ..models import Organization, Customer, CustomerSettings, SubscriptionPlan
...
class OrganizationSeeder:
    ...
    def deploy_organization(self) -> bool:
        ...
        try:
            ...
            self.organization.status = "creating"
            ...
            self.organization.status = "failed"
    ...
    def _create_organization_structure(self):
        ...
        if not self.dry_run:
            ensure_directory_exists(self.config.org_base_path, f"(org root: {self.org_id})")
            ...
            if max_products == -1:  # Unlimited
                product_dirs = ['core-services', 'saas-products', 'partner-products', 'charity-products']
            else:
                # Limit directories based on plan
                product_dirs = ['core-services', 'saas-products']  # Start with core directories
            
            for product_dir in product_dirs:
                ensure_directory_exists(
                    self.config.org_base_path / product_dir, 
                    f"({product_dir})"
                )
    ...
    def _copy_workflow_templates(self, workflows_path: Path):
        ...
        for template_name in workflow_templates:
            template_path = self.config.get_template_path(f"github/workflows/{template_name}")
