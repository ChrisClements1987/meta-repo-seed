    ...
    def check_cli_available(self, cli_command: str) -> bool:
        ...
        try:
            result = subprocess.run(
                [cli_command, "--version"], capture_output=True, text=True, timeout=10
            )
...
class PaaSProviderRegistry:
    ...
    @classmethod
    def get_provider(
        cls, provider_name: str, dry_run: bool = False
    ) -> Optional[PaaSProvider]:
    ...
    @classmethod
    def get_provider_for_template(
        cls, template_name: str, dry_run: bool = False
    ) -> Optional[PaaSProvider]:
...
async def deploy_product_to_paas(
    source_dir: Path, template_name: str, product_name: str, dry_run: bool = True
) -> DeploymentResult:
    """
    ...
    provider = PaaSProviderRegistry.get_provider_for_template(
...
if __name__ == "__main__":
    ...
    async def test_providers():
        ...
        for provider_name in providers:
            ...
            if provider:
                config = PaaSConfig(
                    provider=provider_name, app_name="test-app", config={}
                )
