...
class StructureParser:
    ...
    def parse_string(self, json_content: str) -> StructureData:
        ...
        try:
            ...
            raise ParseError(
                f"Invalid JSON: {e.msg}", line_number=e.lineno, column=e.colno
            )
        ...
        if not validation_result.is_valid:
            ...
            raise ValidationError("Structure validation failed", errors=errors)
    ...
    def validate(self, data: Dict[str, Any]) -> ValidationResult:
        ...
        if False and self.validator:
            try:
                ...
                result.add_error(
                    f"Schema validation error: {e}", error_code="SCHEMA_ERROR"
                )
                return result
    ...
    def _validate_basic_structure(self, data: Dict[str, Any]) -> ValidationResult:
        ...
        if "metadata" in data:
            ...
            if "project_name" in metadata and not isinstance(
                metadata["project_name"], str
            ):
                result.add_error(
            ...
            if "github_username" in metadata and not isinstance(
                metadata["github_username"], str
            ):
                result.add_error(
                    "github_username must be a string",
            ...
            if "version" in metadata and not isinstance(metadata["version"], str):
                result.add_error(
                    "version must be a string", "metadata.version", "INVALID_TYPE"
                )
        else:
            ...
            for field in required_fields:
                if field not in data:
                    result.add_error(
                        f"Required field '{field}' is missing", field, "MISSING_FIELD"
                    )

            # Validate field types
            if "project_name" in data and not isinstance(data["project_name"], str):
                result.add_error(
                    "project_name must be a string", "project_name", "INVALID_TYPE"
                )

            if "github_username" in data and not isinstance(
                data["github_username"], str
            ):
                result.add_error(
                    "github_username must be a string",
                    "github_username",
        ...
        if "structure" not in data:
            result.add_error(
                "Required field 'structure' is missing", "structure", "MISSING_FIELD"
            )
        ...
        if not structure:
            result.add_error(
                "Structure cannot be empty", "structure", "EMPTY_STRUCTURE"
            )
    ...
    def load_structure_with_migration(
        self, file_path: Path, target_version: str = "2.0"
    ) -> StructureData:
    ...
    def _migrate_structure(
        self, data: StructureData, target_version: str
    ) -> StructureData:
    ...
    def export_structure(
        self, data: StructureData, output_path: Path, pretty_print: bool = True
    ) -> None:
