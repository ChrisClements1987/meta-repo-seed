...
class SchemaValidator:
    ...
    def validate(self, data: Dict[str, Any]) -> ValidationResult:
        ...
        try:
            ...
            result.add_error(
                message=f"Schema validation failed: {e}", error_code="SCHEMA_ERROR"
            )
    ...
    def _validate_custom_rules(self, data: Dict[str, Any], result: ValidationResult):
        ...
        if not github_username:
            path = (
                "metadata.github_username" if "metadata" in data else "github_username"
            )
            ...
            result.add_warning(
                "GitHub username format may be invalid", path, "SUSPICIOUS_FORMAT"
            )
        ...
        if not structure:
            result.add_error(
                "Structure definition is required", "structure", "MISSING_FIELD"
            )
        else:
    ...
    def _validate_structure_completeness(
    ...
    def _validate_meta_repo_structure(
        self, meta_repo: Dict[str, Any], result: ValidationResult
    ):
