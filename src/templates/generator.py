...
class ProductTemplateRegistry:
    ...
    TEMPLATES = {
        ...
        "python-api": TemplateConfig(
            ...
            environment_variables=["DATABASE_URL", "SECRET_KEY", "ENVIRONMENT"],
        ),
        "node-api": TemplateConfig(
            ...
            environment_variables=["DATABASE_URL", "JWT_SECRET", "NODE_ENV", "PORT"],
        ),
        "react-spa": TemplateConfig(
            ...
            output_directory="dist",
            environment_variables=["VITE_API_URL", "VITE_AUTH_DOMAIN"],
        ),
...
class ProductTemplateGenerator:
    ...
    def generate_product(
        ...
        """
        self.logger.info(
            f"Generating product: {product_name} from template: {template_name}"
        )
        ...
        template_vars = {
            ...
            "PRODUCT_NAME_TITLE": product_name.replace("-", " ")
            .replace("_", " ")
            .title(),
        ...
        try:
            ...
            self._copy_template_files(
                template_source, product_output_dir, template_vars
            )

            # Generate CI/CD configuration
            self._generate_cicd_config(
                product_output_dir, template_config, template_vars
            )
    ...
    def _generate_ci_workflow(
        self, template_config: TemplateConfig, variables: Dict[str, str]
    ) -> str:
  ...
  test:
    ...
    - name: Install dependencies
      run: |"""
        ...
        if any(
            "Node" in tech or "React" in tech or "Next" in tech
            for tech in template_config.technologies
        ):
            workflow += "\n        npm ci"
                ...
                "builds": [
                    {
                        "src": "package.json",
                        "use": "@vercel/next"
                        if "Next" in template_config.technologies
                        else "@vercel/static-build",
                    }
                ],
                "env": {
                    var: f"@{var.lower()}"
                    for var in template_config.environment_variables
                },
            }
