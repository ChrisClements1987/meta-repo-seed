...
def apply_branch_protection(repo, settings_file, dry_run=False, verbose=False):
    ...
    for branch_config in branches:
        ...
        if dry_run:
            print(
                f"[DRY RUN] Would apply protection to {branch_name}: {protection_data}"
            )
            continue
...
def main():
    ...
    parser.add_argument(
        "--repository",
        default="{{GITHUB_USERNAME}}/{{PROJECT_NAME}}",
    ...
    parser.add_argument(
        "--labels-only", action="store_true", help="Apply only label configurations"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if not args.labels_only:
        success = success and apply_repository_settings(
            args.repository, settings_file, args.dry_run, args.verbose
        )
        success = success and apply_branch_protection(
            args.repository, settings_file, args.dry_run, args.verbose
        )

    # Apply labels (unless settings-only)
    if not args.settings_only:
        success = success and apply_repository_labels(
            args.repository, labels_file, args.dry_run, args.verbose
        )
    ...
    if success:
        ...
        print(
            f"  - View branch protection: gh api repos/{args.repository}/branches/main/protection"
        )
    else:
        print(
            "[ERROR] Some GitHub settings failed to apply. Check the error messages above."
        )
        sys.exit(1)
