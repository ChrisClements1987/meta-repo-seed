...
def mock_templates_dir():
    ...
    (templates_dir / "gitignore.template").write_text(
...
def sample_config_dict():
    ...
    return {
        ...
        "variables": {"ORGANIZATION_NAME": "Test Organization", "LICENSE_TYPE": "MIT"},
        "options": {"verbose": True, "dry_run": False, "backup_existing": True},
    }
...
def mock_file_operations():
    ...
    ) as mock_write, patch("pathlib.Path.exists") as mock_exists:
        mock_exists.return_value = False  # Files don't exist by default
        yield {"mkdir": mock_mkdir, "write_text": mock_write, "exists": mock_exists}
...
def assert_file_created(temp_dir, filename, content_contains=None):
    ...
    if content_contains:
        ...
        ), f"File {filename} does not contain expected content"
