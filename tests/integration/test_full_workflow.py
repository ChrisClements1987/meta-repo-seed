...
class TestCompleteWorkflow:
    ...
    def test_full_repo_seeding_workflow_dry_run(
        self,
        mock_ensure_dir,
        mock_subprocess,
        temp_dir,
        ...
        (mock_templates_dir / "gitignore.template").write_text(
            "# {{PROJECT_NAME}}\n*.log"
        )
        (mock_templates_dir / "readme.template").write_text(
            "# {{PROJECT_NAME}}\n\nBy {{GITHUB_USERNAME}}"
        )
    ...
    @patch("subprocess.run")
    def test_full_repo_seeding_workflow_normal_mode(
        self, mock_subprocess, temp_dir, mock_templates_dir, mock_git_config
    ):
...
class TestMainFunctionIntegration:
    ...
    def test_main_function_with_dry_run(self, mock_username, mock_project, temp_dir):
        ...
        with patch("seeding.Path.cwd", return_value=temp_dir), patch.object(
            RepoSeeder, "run"
        ) as mock_run:
            main()
    ...
    def test_main_function_with_config_file(self, temp_dir, mock_templates_dir):
        ...
        with patch("sys.argv", ["seeding.py", "--config", str(config_file)]), patch(
            "seeding.Path.cwd", return_value=temp_dir
    ...
    @patch(
        "sys.argv",
        ["seeding.py", "--project", "override-project", "--username", "override-user"],
    )
    ...
    def test_git_command_failure_handling(self, mock_subprocess, temp_dir):
        ...
        with patch("seeding.Path.cwd", return_value=temp_dir):
            ...
            try:
                ...
                assert isinstance(
                    e, (subprocess.CalledProcessError, RuntimeError, Exception)
                )
...
class TestConcurrencyAndPerformance:
    ...
    def test_large_project_structure_creation(self, temp_dir):
        ...
        with patch("seeding.Path.cwd", return_value=temp_dir):
            ...
            large_structure = [seeder.project_root / f"service-{i}" for i in range(100)]
...
class TestConfigurationWorkflows:
    ...
    def test_yaml_configuration_workflow(self, temp_dir, mock_templates_dir):
        ...
        with patch("seeding.Path.cwd", return_value=config.base_path):
            seeder = RepoSeeder(
                config.project_name, config.github_username, dry_run=config.dry_run
            )
...
class TestTemplateIntegrationWorkflows:
    ...
    def test_complete_template_processing_workflow(self, temp_dir, mock_templates_dir):
        ...
        with patch("seeding.Path.cwd", return_value=temp_dir):
            seeder = RepoSeeder(
                "template-workflow-project", "template-user", dry_run=False
            )
            seeder.templates_dir = mock_templates_dir
            seeder.run()
            ...
            for template_name, template_content in templates.items():
                ...
                result = create_file_from_template(
                    template_file, output_file, seeder.replacements
                )
    ...
    def test_template_error_recovery_in_workflow(self, temp_dir, mock_templates_dir):
        ...
        (mock_templates_dir / "invalid.template").write_text(
