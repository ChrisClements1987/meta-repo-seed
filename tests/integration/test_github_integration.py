...
class TestGitHubRepositoryOperations:
    ...
    def test_create_github_repository_already_exists(self, mock_run, temp_dir):
        ...
        with patch("seeding.Path.cwd", return_value=temp_dir):
            ...
            result = subprocess.run(
                ["gh", "repo", "create", "existing-repo", "--private"],
                capture_output=True,
                text=True,
            )
        ...
        with patch("seeding.Path.cwd", return_value=temp_dir):
            ...
            result = subprocess.run(
                [
                    ...
                    "edit",
                    "--description",
                    "Updated description for desc-repo",
                ],
                capture_output=True,
                text=True,
            )
...
class TestGitHubIssuesOperations:
    ...
    @patch("subprocess.run")
    def test_create_issue_with_labels_and_assignee(
        self, mock_run, temp_dir, mock_github_cli
    ):
        ...
        with patch("seeding.Path.cwd", return_value=temp_dir):
            ...
            result = subprocess.run(
                [
                    ...
                    "--body",
                    "We need configuration file support for better workflow",
                    "--label",
                    "enhancement,feature",
                    "--assignee",
                    "test-user",
    ...
    def test_close_github_issue(self, mock_run, temp_dir, mock_github_cli):
        ...
        with patch("seeding.Path.cwd", return_value=temp_dir):
            ...
            result = subprocess.run(
                ["gh", "issue", "close", "1", "--comment", "Closing as completed"],
                capture_output=True,
                text=True,
            )
...
class TestGitHubReleaseOperations:
    ...
    def test_create_github_release(self, mock_run, temp_dir, mock_github_cli):
        ...
        mock_run.return_value.returncode = 0
        mock_run.return_value.stdout = (
            "https://github.com/test-user/test-repo/releases/tag/v1.0.0"
        )
    ...
    def test_list_github_releases(self, mock_run, temp_dir, mock_github_cli):
        ...
        with patch("seeding.Path.cwd", return_value=temp_dir):
            ...
            result = subprocess.run(
                ["gh", "release", "list", "--json", "tagName,name,isDraft"],
                capture_output=True,
                text=True,
            )
...
class TestGitHubIntegrationWithSeeder:
    ...
    @patch("subprocess.run")
    @pytest.mark.skip(
        reason="setup_github_repository method not yet implemented - tracked in issue #100"
    )
    ...
    def test_seeder_dry_run_skips_github_operations(self, temp_dir, mock_github_cli):
        ...
        with patch("seeding.Path.cwd", return_value=temp_dir):
            ...
            with patch("subprocess.run") as mock_run:
                ...
                github_calls = [
                    call for call in mock_run.call_args_list if "gh" in str(call)
                ]
