...
class TestSchemaValidator:
    ...
    def valid_data(self):
        ...
        return {
            ...
            "version": "2.0",
            "structure": {
                "meta-repo": {"governance": {"structure": ["structure.json"]}}
            },
        }
    ...
    def test_custom_validation_rules_project_name(self, mock_schema, tmp_path):
        ...
        assert result.error_count == 0 or all(
            "project_name" not in error.path for error in result.errors
        )
    ...
    def test_custom_validation_rules_github_username(self, mock_schema, tmp_path):
        ...
        username_errors = [
            error for error in result.errors if "github_username" in error.path
        ]
