...
class TestAuditManagementSystem:
    ...
    def setup_method(self):
        ...
        self.seeder = RepoSeeder(
            project_name="test-project", github_username="testuser", dry_run=False
        )
    ...
    def test_creates_ai_agent_coordination_configs(self):
        ...
        for config_file in expected_configs:
            ...
            assert (
                "audit_domains" in config_content
            ), "Agent config should specify domains"
            assert (
                "prompt_template" in config_content
            ), "Agent config should have prompt template"
    ...
    def test_creates_audit_template_files(self):
        ...
        for template_file in expected_templates:
            ...
            ), f"Audit template {template_file} should be created"

            # Should contain template structure
            template_content = template_file.read_text()
            assert (
                "## AUDIT SUMMARY" in template_content
            ), "Template should have summary section"
            assert (
                "## FINDINGS" in template_content
            ), "Template should have findings section"
            assert (
                "## RECOMMENDATIONS" in template_content
            ), "Template should have recommendations"
    ...
    def test_audit_agent_coordination_system(self):
        ...
        coordinator_script = (
            project_path / "meta-repo" / "scripts" / "audit-coordinator.py"
        )
        coordinator_content = coordinator_script.read_text()

        # Should coordinate multiple agents
        assert (
            "security-agent" in coordinator_content
        ), "Should coordinate security agent"
        assert (
            "code-quality-agent" in coordinator_content
        ), "Should coordinate code quality agent"
        assert (
            "architecture-agent" in coordinator_content
        ), "Should coordinate architecture agent"
        assert (
            "business-agent" in coordinator_content
        ), "Should coordinate business agent"
    ...
    def test_audit_tracking_prevents_duplicates(self):
        ...
        for tracking_file in expected_tracking_files:
            ...
            ), f"Tracking file {tracking_file} should be created"
    ...
    def test_audit_system_is_idempotent(self):
        ...
        assert len(initial_files) == len(
            final_files
        ), "Audit system creation should be idempotent"
