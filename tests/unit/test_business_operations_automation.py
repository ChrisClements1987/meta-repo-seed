...
class TestBusinessOperationsManager:
    ...
    async def test_setup_automated_onboarding(self, manager, temp_dir):
        ...
        result = await manager.setup_automated_onboarding(
            target_dir=temp_dir, org_name="test-org", repo_name="test-repo"
        )
    ...
    async def test_setup_automated_reporting(self, manager, temp_dir):
        ...
        result = await manager.setup_automated_reporting(
            target_dir=temp_dir, org_name="test-org"
        )
...
class TestBusinessProfileSpecificBehavior:
    ...
    async def test_charity_nonprofit_profile(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = await manager.deploy_complete_automation(
                target_dir=temp_path, org_name="charity-org", repo_name="charity-repo"
            )
...
class TestAutomationLevels:
    ...
    @pytest.mark.parametrize(
        "automation_level,expected_features",
        [
            (
                AutomationLevel.CONSERVATIVE,
                {"self_healing": True, "aggressive_updates": False},
            ),
            (
                AutomationLevel.STANDARD,
                {"self_healing": True, "balanced_approach": True},
            ),
            (
                AutomationLevel.AGGRESSIVE,
                {"self_healing": True, "maximum_automation": True},
            ),
        ],
    )
    @pytest.mark.asyncio
    async def test_automation_levels(self, automation_level, expected_features):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = await manager.deploy_complete_automation(
                target_dir=temp_path, org_name="smb-org", repo_name="smb-repo"
            )
...
class TestFileGeneration:
    ...
    def test_branch_protection_config_generation(self):
        ...
        assert (
            protection_config["main"]["enforce_admins"] == True
        )  # Consulting firm specific
        assert (
            "client-data-protection"
            in protection_config["main"]["required_status_checks"]["contexts"]
        )
    ...
    async def test_deployment_with_template_missing(self):
        ...
        with patch.object(Path, "exists", return_value=False):
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                result = await manager.setup_automated_onboarding(
                    target_dir=temp_path, org_name="test-org", repo_name="test-repo"
                )
