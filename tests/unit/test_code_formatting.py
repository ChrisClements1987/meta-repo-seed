...
class TestCodeFormattingSetup:
    ...
    def test_creates_formatting_requirements_file(self):
        ...
        ), "Formatting requirements file should be created"
    ...
    def test_creates_setup_script_for_developers(self):
        ...
        assert (
            "pre-commit install" in script_content
        ), "Script should install pre-commit hooks"
        assert (
            "pip install" in script_content
        ), "Script should install required packages"
    ...
    def test_creates_pyproject_toml_configuration(self):
        ...
        assert (
            "[tool.flake8]" in pyproject_content or "[tool.ruff]" in pyproject_content
        ), "Should configure linter"
        ...
        for repo in config_data.get("repos", []):
            if "rev" in repo:
                ...
                assert rev not in [
                    ...
                    "master",
                    "latest",
                ], f"Should pin specific version, not {rev}"
                assert (
                    len(rev) > 3
                ), "Version should be specific (not just v1 or similar)"
        ...
        if "line-length = " in pyproject_content:
            ...
            for line in lines:
                if "line-length = " in line and "[tool.black]" in pyproject_content:
                    ...
                    assert (
                        "88" in pyproject_content or "100" in pyproject_content
                    ), "Should have reasonable line length"
    ...
    def test_setup_script_is_cross_platform(self):
        ...
        assert (
            "import sys" in script_content or "import os" in script_content
        ), "Should use platform-aware code"
        assert (
            "python -m pip" in script_content or "sys.executable" in script_content
        ), "Should use proper Python executable"
    ...
    def test_formatting_setup_is_idempotent(self):
        ...
        assert len(initial_files) == len(
            final_files
        ), "Code formatting setup should be idempotent"
