...
class TestGitHubRepositorySettings:
    ...
    def test_creates_repository_settings_config(self):
        ...
        settings_config = (
            project_path / "meta-repo" / ".github" / "repository-settings.yml"
        )
    ...
    def test_creates_branch_protection_rules(self):
        ...
        for keyword in protection_keywords:
            assert (
                keyword in settings_content
            ), f"Should include branch protection: {keyword}"
    ...
    def test_creates_github_settings_automation_script(self):
        ...
        automation_script = (
            project_path / "meta-repo" / "scripts" / "apply-github-settings.py"
        )
        assert (
            automation_script.exists()
        ), "GitHub settings automation script should be created"
        ...
        assert (
            "gh repo edit" in script_content
        ), "Script should use GitHub CLI for repository settings"
        assert (
            "gh api" in script_content
        ), "Script should use GitHub API for advanced settings"
    ...
    def test_repository_settings_use_template_variables(self):
        ...
        assert (
            "{{PROJECT_NAME}}" not in settings_content
        ), "Template variables should be replaced"
        assert (
            "{{GITHUB_USERNAME}}" not in settings_content
        ), "Template variables should be replaced"
    ...
    def test_creates_github_workflow_for_settings_automation(self):
        ...
        assert (
            "repository-settings" in workflow_content
        ), "Workflow should handle repository settings"
    ...
    def test_settings_include_security_configurations(self):
        ...
        for setting in security_settings:
            ...
            ), f"Should include security setting: {setting}"
    ...
    def test_github_settings_are_idempotent(self):
        ...
        assert len(initial_files) == len(
            final_files
        ), "GitHub settings creation should be idempotent"
