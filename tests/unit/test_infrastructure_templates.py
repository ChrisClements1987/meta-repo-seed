...
class TestInfrastructureTemplates:
    ...
    def test_creates_terraform_provider_templates(self):
        ...
        for expected_file in expected_files:
            ...
            ), f"Terraform template {expected_file} should be created"
    ...
    def test_creates_kubernetes_manifest_templates(self):
        ...
        for expected_file in expected_files:
            assert (
                expected_file.exists()
            ), f"Kubernetes template {expected_file} should be created"
            assert expected_file.is_file(), f"{expected_file} should be a file"
    ...
    def test_creates_docker_templates(self):
        ...
        for expected_file in expected_files:
            assert (
                expected_file.exists()
            ), f"Docker template {expected_file} should be created"
            assert expected_file.is_file(), f"{expected_file} should be a file"
    ...
    def test_creates_environment_specific_configs(self):
        ...
        for env_dir in expected_dirs:
            assert (
                env_dir.exists()
            ), f"Environment directory {env_dir} should be created"
    ...
    def test_template_variable_replacement(self):
        ...
        main_tf = (
            project_path / "meta-repo" / "infrastructure" / "terraform" / "main.tf"
        )
        content = main_tf.read_text()

        # Should contain project-specific values, not template variables
        assert (
            "test-project" in content
        ), "Project name should be replaced in terraform templates"
        assert (
            "testuser" in content
        ), "GitHub username should be replaced in terraform templates"
        assert (
            "{{PROJECT_NAME}}" not in content
        ), "Template variables should be replaced"
        assert (
            "{{GITHUB_USERNAME}}" not in content
        ), "Template variables should be replaced"
    ...
    def test_infrastructure_templates_idempotent(self):
        ...
        initial_file_count = len(
            list((project_path / "meta-repo" / "infrastructure").rglob("*"))
        )
        ...
        ), "Infrastructure template creation should be idempotent"
    ...
    def test_cross_platform_infrastructure_templates(self):
        ...
        ), "Infrastructure templates should use cross-platform Path objects"

        # File should be readable across platforms
        content = terraform_main.read_text()
        assert (
            len(content) > 0
        ), "Infrastructure template content should be generated correctly"
