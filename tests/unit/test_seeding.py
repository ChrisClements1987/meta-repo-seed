...
class TestRepoSeeder:
    ...
    def test_run_method_dry_run(self, temp_dir, mock_templates_dir):
        ...
        with patch("seeding.Path.cwd", return_value=temp_dir), patch.object(
            RepoSeeder, "create_base_structure"
        ) as mock_base, patch.object(
            RepoSeeder, "setup_cloud_storage"
        ) as mock_cloud, patch.object(
            RepoSeeder, "setup_meta_repo"
        ) as mock_meta:
            seeder = RepoSeeder("test-project", "test-user", dry_run=True)
...
class TestUtilityFunctions:
    ...
    def test_get_github_username_fallback_to_input(
        self, mock_input, mock_run, mock_isatty
    ):
    ...
    def test_get_github_username_raises_error_on_empty_input(
        ...
        with pytest.raises(
            ValueError,
            match="GitHub username not provided and no non-interactive source available",
        ):
            get_github_username()
...
class TestTemplateProcessing:
    ...
    @patch("seeding.ensure_directory_exists")
    def test_setup_meta_repo_creates_gitignore(
        self, mock_ensure_dir, mock_copy_template, temp_dir, mock_templates_dir
    ):
        ...
        with patch("seeding.Path.cwd", return_value=temp_dir):
            ...
            with patch.object(seeder, "init_git_repo"), patch.object(
                seeder, "create_gitignore"
            ) as mock_gitignore:
    ...
    def test_template_replacements_applied_correctly(
        self, temp_dir, mock_templates_dir, sample_replacements
    ):
        ...
        result = create_file_from_template(
            template_file, output_file, sample_replacements
        )
